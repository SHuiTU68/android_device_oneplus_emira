...
# 不使用 GKI 模式（BOARD_USES_GENERIC_KERNEL_IMAGE），因为 GKI 会在
# first_stage_ramdisk/system/ 下添加 libc.so 等库文件，原厂没有这些文件，
# 可能导致 init 启动流程异常和 bootloop。
# 通过 BOARD_VENDOR_RAMDISK_KERNEL_MODULES 确保模块进入 platform ramdisk。
